...
describe('User Routes', () => {
    ...
    it('should not signup user with invalid email', async () => {
        const res = await request(app)
            .post('/api/v1/signup')
            .send({email: 'invalid_email', password: 'password@123'});
    ...
    it('should not signup user with existing email', async () => {
        ...
        const res = await request(app)
            .post('/api/v1/signup')
            .send({email: 'exiting.email@example.com', password: 'password@123'});
    ...
    it('should login a user', async () => {
        ...
        const res = await request(app)
            .post('/api/v1/login')
            .send({email: 'should.login@example.com', password: 'password@123'});
